...
const state = {
  meta: null, mode: "verified", gemOnly: false, sort: "trust", featureMode: "prefer",
...
const SITUATIONS = [
  // hardFeat = 반드시 있어야 하는 조건(요약칩으로 노출, 0곳이면 완화 제안). prefFeat = 우대(가점).
  { key: "kids", emo: "🍼", lb: "아이랑", hardFeat: ["kidFriendly", "parking"], prefFeat: ["kidsCafe"] },
  { key: "party", emo: "👥", lb: "회식", prefFeat: ["partition", "privateRoom", "parking"] },
  { key: "date", emo: "💕", lb: "데이트", tags: ["데이트"], pref: ["분위기좋은", "조용한", "고급스러운"] },
  { key: "solo", emo: "🍚", lb: "혼밥", hardFeat: ["soloFriendly"] },
...
async function init() {
  ...
  $("featMode").querySelectorAll("button").forEach((b) => (b.onclick = () => { setFeatMode(b.dataset.m); clearSituation(); run(); }));
...
function clearFilters() {
  ...
  setFeatMode("prefer"); syncPills();
}
function setFeatMode(m) {
  state.featureMode = m;
  $("featMode") && $("featMode").querySelectorAll("button").forEach((b) => b.classList.toggle("on", b.dataset.m === m));
  const h = $("featHint");
  if (h) h.textContent = m === "require" ? "‘꼭 필요해요’는 그 조건이 없는 곳은 결과에서 빼요." : "‘있으면 좋아요’는 조건이 맞는 곳을 위로 올려줘요(빼지 않아요).";
}
...
function pickSituation(s, btn) {
  ...
  setFeatMode(s.hardFeat && s.hardFeat.length ? "require" : "prefer");
  (s.hardFeat || []).forEach((f) => state.sel.features.add(f));
...
function buildQuery(extra = {}) {
  ...
  const manual = [...state.sel.features], hard = state.featureMode === "require";
  const require = {}, preferF = {};
  manual.forEach((f) => ((hard ? require : preferF)[f] = true));
  (state.preferFeatures || []).forEach((f) => (preferF[f] = true));
  const q = {
    styles: [...state.cats], cuisines: [...state.sel.cuisines], tagsAny: [...state.sel.tags],
    menuAttrs: [...state.sel.menuAttrs], require, preferFeatures: preferF, prefer: state.prefer,
...
function renderCount(meta, n) {
  const where = meta.origin ? `${$("locName").textContent} 기준` : "전국";

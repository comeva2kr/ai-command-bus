...
2. **법적 안전 모델 유지** (`docs/legal.md`): 아웃링크 필수(프레이밍 금지), 발췌 ≤200자, 본문 복제 금지, 대량 DB 복제 금지. 화제성은 공개 신호(추천/댓글 수)로만.
   **수집 계층 (2026-07-23 David 최종 지시로 개정 — jagei.co.kr 방식 채택)**: ① 공식 RSS/API 우선 → ② 없으면 **베스트보드 리스트 페이지에서 제목+링크+시각+공개지표만 파싱** (본문 미수집, **이미지는 URL 핫링크 참조만 — 저장·재호스팅 안 함, 2026-07-26 개정, 아래 참고**, 폴링 주기 ≥10분, 식별 UA, 사이트당 리스트 1~2페이지만). ③ **제외 확정 (David 최종 지시 2026-07-23): 에펨코리아·아카라이브·디시인사이드·일베는 소스로 넣지 않는다** — 차단 여부와 무관하게 시도 자체 금지. 이 개정은 robots 엄격 준수보다 제목 수준 어그리게이션 관행(자게이 등 다수 서비스)을 따르는 WARN 수용 결정이며, 출시 전 변호사 검토 목록에 포함.
   **썸네일 이미지 (2026-07-26 David 지시로 추가)**: 이미지는 저장/재호스핑 절대 금지, 원본 서버 URL만 핫링크(카톡/트위터 링크 미리보기 방식) — 상세는 `docs/legal.md` "Images: hotlink only" 참고. RSS는 그 항목이 이미 실어 온 `media:thumbnail`/`media:content`/이미지-타입 `enclosure`/description·content 내 첫 `<img>`에서, list 소스는 communities.json의 소스별 `thumbRegex`(행 안 자체 썸네일 `<img>`)에서 뽑는다 — 추가 fetch 없음. 실측(2026-07-26): theqoo/bobae/ppomppu/todayhumor/etoland/inven_hot은 리스트 HTML에 파일타입·HOT 배지 아이콘만 있고 실제 글 썸네일이 없어 의도적으로 thumbRegex 미설정(이미지 없음이 정직한 기본값) — 유일하게 pann(현재 robots 사유로 비활성)만 행마다 자체 썸네일을 렌더해 thumbRegex 적용. 라이브 실측(로컬, FEED_LIVE=1): clien·ruliweb·ddanzi·slownews·yozm·outstanding·techmeme RSS 소스에서 실제 썸네일 확인(예: ruliweb i-cdn, techmeme 자체 이미지, ddanzi/slownews WordPress 본문 이미지). gnews 계열·geeknews·devto·hackernews·slashdot·tildes·44bits·newspeppermint는 원본 피드 자체에 이미지가 없어 텍스트 카드 유지.
   ④ **열람 방식 (David 최종 지시 2026-07-23, jagei 실측 기반)**: iframe 허용 사이트는 상단바+iframe 프레임 뷰어(자게이 동일), 차단 사이트는 새 탭 아웃링크. 프레이밍은 legal.md가 경고한 상위 리스크 항목 — **변호사 검토 목록 최우선 등재** (임베드 표시 판례 쟁점). 본문 복제·저장은 여전히 금지(iframe은 원 서버가 직접 서빙).
...
1. **국내 호스팅 이전 — Oracle Cloud 무료 티어 서울 리전 (David 승인됨, 2026-07-23)**
   - 선행: David가 Oracle 계정 가입(폰/PC, 구글 계정 연동 의향 — 단 OCI 가입은 카드 인증이 별도로 필요할 수 있음) 후 맥 크롬에서 cloud.oracle.com 로그인 1회. 이후는 세션이 전부 진행.
   - 목적: ① 해외 IP 차단으로 죽은 todayhumor·humoruniv·mlbpark 소스 부활(이전 전 한국 데이터센터 IP에서 fetch 테스트 먼저) ② `FEED_DB` 영속화(무료 VM 디스크) ③ 상시 가동(스핀다운 제거) ④ 월 비용 0원.
   - 절차: 서울 리전 무료 ARM VM 생성 → 차단 3소스 fetch 테스트 → Docker/systemd 셋업 + Caddy HTTPS → 데이터 이전 → DNS 전환(도메인 미보유 시 임시로 무료 서브도메인, 도메인 구매는 David 결정) → Render는 검증 후 정리.
2. 실기기 웹푸시 E2E 수동 확인 (iPhone/Android 브라우저에서 알림 받기 → `/api/admin/push-digest` 수동 발사).
3. `docs/monetization.md` Phase A: 제휴 카드 슬롯 구현 (약관 확인 후).
4. ~~Render 유료 플랜~~ → 1번(국내 이전)으로 대체.

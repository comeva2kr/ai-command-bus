...
| aggregate(종합 tier 의미 분류) | **PASS** (2026-08-13 HOLD 해소, 커밋 a970603) | 당초 HOLD 사유: aggregate 소스(gnews-biz 등)의 제목 의미 분류 실통과 미구현. 해소: aggregateReclassification 관문(약한 prior — NB 기존 운영점 재사용, 가드·UNTRAINED 관문 요구, 감사 기록·매 사이클 재판정) + 동결 테스트(실물 오분류 교정 3·정분류 유지 4). 운영 조건 전수 재분류 0/112 — 과교정 없음. LOO 턱걸이 1건은 감사 로그 모니터링 항목. |

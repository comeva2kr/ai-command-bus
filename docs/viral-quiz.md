...
테스트 구조(축 기반 스펙트럼 채점, 9~13문항, 강점 80:약점 20, 궁합, 희소성
통계, 면책 라벨)는 국내외 히트 테스트 딥리서치에서 도출한 명세를 따른다 —
[docs/quiz-design.md](quiz-design.md) 참고. `validateQuiz`가 이 명세를 생성
결과에 강제하므로, AI가 명세를 벗어난 퀴즈를 만들면 초안 단계에서 거부된다.

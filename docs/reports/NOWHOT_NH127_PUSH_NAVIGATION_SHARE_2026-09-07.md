...
- 현재 상태: Root 통합 구현·독립 검토 PASS, Fable/Grok 조사 회수·완료 워커 release·ACK. 운영 배포 준비 완료이며 아래 영수증이 추가되기 전까지 미배포이다.
...
- 변경 전 운영 `afee38acb876515e3b59ae50694ca872876eb6f1`, 점검1시간·VAPID 키 존재 확인. 저장 사용자1213·실제 endpoint 구독4·활성4를 개인정보 없이 집계했다. 구독4개 중 성공 기록 없는2개를 특정 사용자 기기라고 단정하지 않는다. 실제 고객을 대상으로 시험 발송하지 않았다.
...
- 개발현황 반영: 위 안정 ID/변경 레코드로 최종 운영 영수증과 대조 후 기록한다. 현재는 미배포이다.
...
- 하지 않은 일: 자체 기사 수집/작성 운영 가동·광고 심사 신청·미확인 실기기 완료 주장·전체 풀 재수집·정본 재발행.

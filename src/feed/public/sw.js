...
const CACHE = "feed-shell-v139"; // v139: 홈 자체 서술 — 리포트 한 줄(ob-story) // v138: 삼성 번역 안내 정정(브라우징 어시스트) + 웨일·엣지·FF 분기 // v137: 번역 안내를 최신 UI 실기기 기준으로(사파리 기능버튼·iOS크롬 ⋯) // v136: 네이버앱 버튼 아래 상단-번역 1탭 안내 // v135: 얀덱스 제거(통신사 차단 실기기 확정)→네이버앱 딥링크 후보 // v134: 원문 전체 한국어 버튼(얀덱스 프록시, KR 실측 통과) // v133: 크롬 안내를 ⋮ 메뉴 경로로(자동 프롬프트 비의존) // v132: 번역 안내를 접속 브라우저별 맞춤으로 // v131: 번역 안내에 항상-번역(1회 설정=이후 자동) 추가 // v130: 전체번역 링크 제거(3사 전부 사망 확인)→브라우저 번역 안내 // v129: 누른 글은 무조건 열림(카드 데이터 폴백) + 소스·카테고리 새로고침 고정 // v128: 원문 한국어 링크를 구글 번역 프록시로 교체(파파고 딥링크 폐지 확인) // v127: 카테고리 재요청 무한루프(스피너만 계속) 수정 // v126: 해외 글에 파파고 원문 번역 링크 // v125: 카테고리를 서버에서 거른다 + 라이트모드 시인성 // v124: 내 공간에 내 발자취(최근 본 글 횡스크롤) // v123: 급등은 즉시 반영, 하락만 완만 // v122: 목록 안정화(점수 관성) + 자리별 클릭 집계 복구 // v121: 카드 원문 링크를 진짜 앵커로(크롤러 가시성) // v120: 홈에 원문 링크 노출 + 준비중 표기 제거(애드센스) // v119: 상세 헤더 글자 밀림(버튼 안 줄바꿈) // v118: 신설 카테고리가 분류기에 덮어써지던 것 // v117: 원문 언어를 실제 감지값으로(KO 표기 제거) // v116: 카테고리 3개 신설(부동산·패션·예술) // v115: 해외 글 — 같은 뜻 버튼 중복 제거, 누를 게 없으면 버튼 안 보임 // v114: 해외 글 — 원문 링크 3개 정리 + 옮긴 것이 제목뿐이면 그렇게 말한다 // v113: 화제도 그래프를 누적값으로(로딩중만 보이던 것) // v112: 해외 글 원문 보기·번역본 보기 토글 // v111: 업데이트 소식 알림(바뀐 뒤 처음 온 사람에게 한 번) // v110: 온보딩 오버레이가 메뉴 클릭을 먹던 것 수정 // v109: 메뉴 핫딜 버튼 제거 + 콘텐츠 필터에 핫딜 숨기기 // v108: (서버 변경 — 방문자 표식) // v107: 드로어에 데이터 리포트 // v106: 관련글 링크 구분 // v105: 상세에 편집 코멘트 + 같은 사건 다른 소스 // v104: 설정에 핫딜 모아보기 버튼 + 화면 테마를 내 공간으로 // v103: 몰입 모드 광고 카드 레이아웃 복구 // v102: 핫딜 모아보기 탭 + 우리 딜이 경쟁에 안 밀림 // v101: 우리 딜 카드에 확인 시점 표시 // v100: 우리 딜이 광고 카드 경로를 타서 고지문·가격이 화면에 나온다 // v99: 차단기에 본문이 통째로 지워지던 것 복구(사진만 빠지고 글자는 남게) // v98: 광고 차단기가 본문만 지웠을 때 빈 껍데기를 남기지 않는다 // v97: 빌드 태그 없는 옛 화면은 워커 해제 후 한 번에 갱신 // v96: 낡은 화면이 스스로 갱신한다(빌드 대조) // v95: 광고 썸네일 크기를 자리와 무관하게 통일(피드 88 / 상세 76이던 것) // v94: 광고 카드 렌더러를 하나로 통합(서버·화면 두 경로가 다르게 생기던 것) // v93: 첫 화면에 화제글이 보이게 자체 콘텐츠 높이 예산 + 중복 라벨 제거 + 캡값 숫자 제거 // v92: 홈 앞자리에 우리가 쓴 브리핑 본문 펼침(애드핏 P0-A 자체 콘텐츠 비중) // v91: 앱 광고 카드가 LLM 문구(line·cta)를 실제로 쓴다 // v90: 브리핑 알림 호흡 주기 5초 → 3초 // v89: 브리핑 알림은 대표 카드 한 장만 호흡(스트립 전체 맥동 완화) // v88: 새 브리핑 편성 은은한 알림(눌러서 보기 전까지) + 브리핑 본문 사이사이 연관 광고 // v87: 애드핏 지면을 목록 중간 전용 카드로(하단이면 노출 0) // v86: 애드핏 지면을 목록 끝 전용 카드로 분리(쿠팡 슬롯 보존 + 심사 지면 유지) // v85: 애드핏 빈 지면이 쿠팡 자리 먹던 것 수정 + 딜 상한·간격 + 모든 글에 연관 광고 // v84: 핫딜 소스 5곳 + 딜 최소 비율 보장 + 딜 상품군에 맞춘 광고(피드·상세) // v83: 광고 카드를 게시글과 같은 칸으로(색·테두리·CTA) + 문구 3단 // v82: 광고 썸네일을 콘텐츠와 같은 88px로 + 배너 캐시 갱신 // v81: 광고 카드를 콘텐츠 카드 모양으로(정사각 썸네일) // v80: 애드핏 지면 유지(폴백이 지우지 않음) // v79: 광고 클릭 보고 복구 + 광고 인접 안전검사 // v78: 정치·종교 버튼 복구 + 취향 현재값 표시 + 고지문 시인성 // v77: 자리 복원 판별을 체류시간 기준으로 재설계(검수 P0) // v76: 죽은 번역 링크 제거·혼합 언어 정리·브리핑 편성 표시 // v75: 읽던 자리로 돌아오기(강제 재로드 복원) // v74: 브리핑 편성 화면 완성(슬롯 레일·날짜 이동) // v73: IndexNow 배선 + 설명문 정정(죽은 소스 제거·80자) // v72: 유입 경로 개방(Discover·카톡 공유·색인 정리) // v71: 조회수·작성일 수집 + 브리핑 날짜·시간대 이동 // v70: 하루 3편 브리핑 편성(아침·점심·저녁) + 랭킹 순위 번호 // v69: 승격 제외·광고 인접 가드·제목 번역 복구·한글 번역 링크 (2026-08-04) // v68: 광고 지면 정상화·홈 서버렌더·신원 확정·약관/방침 보강 (2026-08-04)

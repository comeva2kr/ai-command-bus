...
// 세션 총량 상한은 **두지 않는다** (-1 = 무제한).
//
// 2026-07-25에 6으로 넣었다가 08-06에 40으로 올렸고, 결국 없앴다.
// David: "광고에 상한선이 왜 필요해? 일정 컨텐츠 → 광고는 계속 반복되어야지.
//         너무 자주 나오지 않는 선에서, 어차피 디자인이 통일이라 눈에 엄청
//         거슬리지도 않으니까."
//
// 맞다. 촘촘함을 막는 일은 **밀도 규칙**이 이미 한다 — 9칸마다 1개, 요청당
// 최대 2개, 앞 6칸 보호. 그 위에 총량을 또 걸면 하는 일이 하나뿐이다:
// 많이 읽는 사람일수록 광고를 덜 보게 만드는 것. 그건 보호가 아니라 손실이다.
// 실제로 6일 때는 12페이지를 넘겨도 6건에서 멈췄다.
//
// 폭주 방지는 요청 단위(maxPerPage)가 맡는다. 무한 삽입은 한 요청 안에서
// 일어나지 여러 요청에 걸쳐 일어나지 않는다.
// 필요하면 AD_MAX_PER_SESSION 환경변수로 언제든 되살릴 수 있고,
// 0을 넣으면 광고를 완전히 끈다.
const AD_MAX_PER_SESSION_DEFAULT = -1;

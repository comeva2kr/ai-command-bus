...
import { classifyTopics, FILTERABLE_TOPICS, NO_DEAL_TOPIC } from "./topics.js";
...
function topicsBlocked(item, showTopicsSet) {
  const topics = item.topics = classifyTopics(item);
...
export class FeedEngine {
  ...
  _classifyItems(items) {
    ...
    for (const item of items) {
      if (item) item.topics = classifyTopics(item);

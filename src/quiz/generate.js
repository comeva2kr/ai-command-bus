...
export function buildPrompt(topics, opts = {}) {
  ...
  return [
    ...
    "- 같은 토픽을 두 문항에 쓰지 마라 — 토픽당 문항 1개.",
    "- 연예인·특정 동네처럼 모를 수 있는 소재 문항엔 '누군지/뭔지 몰라서 그냥 넘긴다' 류 중립 선택지를 1개 넣어라(그 선택지도 pole은 가져야 함).",
    ...
    "- shareText는 60자 이내, '나는'으로 자기 유형을 선언하되 나머지는 유형마다 완전히 다른 드립으로 써라. 8개를 나란히 놓았을 때 같은 틀이 보이면 실패다. (예: '나는 인간 알림봇 — 네 소식 나한테 먼저 옴. 너는?' / '나는 조용한 얼리어답터, 알고도 조용히 있는 중. 너는?')",
    "",
    "## 금지",
...
export function templateQuiz(topics, opts = {}) {
  ...
  // t0: 이번 주 1순위 토픽의 핵심어 — 제목/결과문에 그대로 박아 QG2의 토픽
  // 인용 검사(범용 제목/결과문 금지)를 코드로도 만족시킨다.
  const t0 = topics[0].title.replace(/\s+/g, " ").trim().slice(0, 10);
  const results = [
    {
      code: "FS",
      title: "속보 그 자체, 인간 알림봇",
      description: `"${t0}" 뜨자마자 단톡방 세 군데에 이미 뿌렸다. 화제가 되기 전에 반응하고, 반응한 순간 다 퍼뜨리는 타입 — 네 주변인들은 뉴스 앱이 필요 없다.`,
      ...
      bestMatch: "TK",
      worstMatch: "TS",
      shareText: "나는 인간 알림봇 — 네 소식 나한테 먼저 옴. 너는?"
    },
    {
      code: "FK",
      title: "조용한 얼리어답터",
      description: `"${t0}" 뜬 거 남들보다 먼저 봤는데 굳이 말 안 한다. 어느 날 대화 중에 "아 그거? 옛날에 봤는데"가 자연스럽게 나오는 타입.`,
    ...
    },
    {
      code: "TS",
      title: "팩트로 무장한 전파자",
      description: `"${t0}"가 진짜인지 원문부터 까보고, 확신이 서면 그때 제일 크게 퍼뜨린다. 네 공유엔 신뢰가 붙는다 — 주변의 팩트체커.`,
      ...
      bestMatch: "FK",
      worstMatch: "FS",
      shareText: "나는 팩트로 무장한 전파자. 출처 없이 말 걸지 마라. 너는?"
    },
    {
      code: "TK",
      title: "느긋한 큐레이터",
      description: `"${t0}" 같은 건 북마크에 고이 모셔두고 일주일 뒤에 본다. 시간이 지나도 남을 것만 골라 담는 타입 — 네 북마크 폴더는 박물관급.`,
  ...
  const quiz = {
    title: `"${t0}" 뜬 날 내 반응 유형은?`,
    description: `이번 주 "${t0}" 같은 화제들 앞에서 네 반응 축 2개를 재본다. 4가지 중 넌 어디?`,
    axes,
    questions,
    results
  };

...
test("old pool links migrate independently of feed freshness and survive the HTTP detail/share routes", async (t) => {
  ...
  let fetches = 0;
  const server = createServer({ file: f.file, sources: [{ id: "publisher", async fetch() { fetches++; return []; } }] });
  ...
  assert.match(await share.text(), /Article old-public/);
  ...
  assert.equal((await restricted.json()).code, "TOPIC_FILTERED");
